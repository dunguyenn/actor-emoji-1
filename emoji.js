...
function emoji(){}
	...
	/**
	 * @memberof emoji
   * @param {string} str A string to change emoji.replace_mode
	 */
	emoji.change_replace_mode = function(str){
    emoji.init_env();

		var enabledReplaceModes = ['img', 'css', 'unified', 'google', 'softbank'];
		if (enabledReplaceModes.includes(str)) {
			emoji.replace_mode = str;
		}
	};
    ...
    wrapper = wrapper || emoji.include_text ? ':' : '';
    ...
    var title = emoji.include_title ? ' title="'+wrapper+(actual || emoji.data[idx][3][0])+wrapper+'"' : '';
